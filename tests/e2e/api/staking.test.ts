...
import { CedraConfig, Cedra, Network } from "../../../src";
import { longTestTimeout } from "../../unit/helper";

// TODO: We don't have delegation pools for now
describe("staking api", () => {
  ...
  //     const numDelegatorsData = await cedra.getNumberOfDelegatorsForAllPools({
  ...
  //       );
  //     }
  //     const numDelegators = await cedra.getNumberOfDelegators({ poolAddress: numDelegatorsData[0].pool_address! });
  ...
  test("it returns 0 if the poolAddress does not exist", async () => {
    ...
    const numDelegators = await cedra.getNumberOfDelegators({ poolAddress: badAddress });
  ...
  //   const delegatedStakingActivities = await cedra.getDelegatedStakingActivities({
  //     poolAddress,
  //     delegatorAddress,
  //   });
